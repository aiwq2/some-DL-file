...
class LSTMModel(BasicModule):
    ...
    def forward(self, input, hidden = None):
        ...
        embeds = self.embeddings(input)# (batch_size, max_len,embedding_dim)
        ...
        output, hidden = self.lstm(embeds, (h_0, c_0))# (batch_size, max_len,hidden_dim)
        output = self.fc(output)# (batch_size, max_len, 2)
        #取最后一个时间步的输出
        last_outputs = self.get_last_output(output, batch_size, max_len)# (batch_size, 2)
